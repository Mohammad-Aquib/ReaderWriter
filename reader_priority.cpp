...
void update_balance(int amount)
{
    ...
    std::cout << "-->Updated balance is:" << total_money << "no .of writers" << writers_count << "::no.of readers:" << readers_count << ":readers_waiting:" << readers_entered << ":writers_waiting:" << writers_entered;
    std::cout << "\n";
    writers_count--;
    w_mutex.unlock();
    y.unlock();
